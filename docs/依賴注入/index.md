...
在開發/測試/正式三個執行環境，常常需要各自使用不同資料來源、或是使用不同連線字串，是開發系統時常見的功能需求。例如：開發環境使用Mock資料來源(假資料)、測試環境使用SQL資料來源(連線至測試資料庫)、正式環境使用SQL資料來源(連線至正式資料庫)。本篇範例協助開發人員使用MDP.Hosting，逐步完成必要的設計和實作。
