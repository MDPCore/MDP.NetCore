...
在MDP.Net核心模組中，「服務注入模組」注入使用ServiceAttribute註冊的Instance時，會參考Configuration參數設定，決定注入的Instance為TypedInstance、或是NamedInstance(參考：[服務註冊(ServiceAttribute)](../../Kernel/服務註冊(ServiceAttribute)/服務註冊(ServiceAttribute).html))。依照本節的類別宣告及參數設定，就可以在系統裡註冊Type(DecorateDemoService)為自己的Instance，並且依照下列規則逐層注入Service(DemoService)。
